...
class OpportunityTypeNormalizationTests(unittest.TestCase):
    def test_normalizes_source_specific_types_for_feed_display(self):
        cases = [
            ("govwin_export", "Solicitation", "Solicitation"),
            ("sam", "Combined Synopsis/Solicitation", "Solicitation"),
            ("sam", "Sources Sought", "RFI"),
            ("sam", "Special Notice", "RFI"),
            ("sam", "Presolicitation", "RFI"),
            ("grants_gov", "Funding Opportunity", "Solicitation"),
            ("sam", "Unmapped Notice", "Solicitation"),
        ]

        for source, raw_type, expected in cases:
            with self.subTest(source=source, raw_type=raw_type):
