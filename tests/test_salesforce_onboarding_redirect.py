...
from bidlens.models import Organization, OrganizationMembership, User
...
class SalesforceOnboardingRedirectTests(unittest.TestCase):
    ...
    def tearDown(self):
        api_routes._SALESFORCE_OAUTH_PKCE.clear()
        self.db.close()
        self.engine.dispose()
    ...
    def test_pre_live_salesforce_oauth_completion_returns_to_organization_setup(self):
        api_routes._SALESFORCE_OAUTH_PKCE["state-token"] = "verifier"
        service = Mock()
        ...
        with (
            ...
            patch("bidlens.routes.api.attach_request_user_context", return_value=self.admin),
        ):
