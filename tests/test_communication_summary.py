...
class CommunicationSummaryTests(unittest.TestCase):
    ...
    def test_template_has_header_actions_unified_conversation_and_metadata(self):
        ...
        self.assertEqual(html.count("team_summary_card("), 0)
        self.assertEqual(html.count("{{ communication_summary.current_status }}"), 1)
        self.assertIn("View full summary", html)
        self.assertIn('name="return_tab" value="overview"', html)
        ...
        communication = html[html.index('data-detail-panel="communication"'):html.index('data-detail-panel="notes"')]
